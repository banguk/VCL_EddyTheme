...
void __fastcall TEddyBaseControl::MouseDown(TMouseButton Button, TShiftState Shift, int X, int Y)
{
    if (Button == mbLeft)
    {
        isMouseDown_ = true;
        // 포커스 획득 (WinForms/WPF 동작 모방)
...
//---------------------------------------------------------------------------
void __fastcall TEddyBaseControl::MouseUp(TMouseButton Button, TShiftState Shift, int X, int Y)
{
    if (Button == mbLeft)
    {
        isMouseDown_ = false;
...
void __fastcall TEddyBaseControl::WMSetFocus(TWMSetFocus &Message)
{
    isFocused_ = true;
    Invalidate();
...
//---------------------------------------------------------------------------
void __fastcall TEddyBaseControl::WMKillFocus(TWMKillFocus &Message)
{
    isFocused_ = false;
    Invalidate();
    TCustomControl::Dispatch(&Message);
...
//---------------------------------------------------------------------------
void __fastcall TEddyBaseControl::WMEraseBkgnd(TWMEraseBkgnd &Message)
{
    // VCL 기본 배경 지우기를 차단하여 깜빡임 제거 (DoubleBuffered와 함께 사용)
    Message.Result = 1;
}
...
TColor __fastcall TEddyBaseControl::GetCurrentBackgroundColor()
{
    // 편의 함수: 현재 상태에 적절한 배경색 반환
    const TEddyPalette &p = GetTheme()->GetPalette();
    if (isMouseDown_) return p.Accent; // 클릭 시
    if (isMouseOver_) return p.Hover;  // 호버 시
    return p.Surface;                  // 평상시
}
